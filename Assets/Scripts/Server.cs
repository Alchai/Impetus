...
public class Server : MonoBehaviour
{
    ...
    public void SendInput(int index, bool down_or_up, int seshID, bool p1_p2, Vector3 snapPos)
    {
        foreach (Session s in activeSessions)
            ...
            {
                if (!p1_p2)
                    networkView.RPC("SendInput", s.p1, index, down_or_up, seshID, !p1_p2, snapPos);
                else
                    networkView.RPC("SendInput", s.p2, index, down_or_up, seshID, p1_p2, snapPos);
            }
    }
    ...
    public void EndTheGame(int mysID)
    {
        ...
        for (int i = 0; i < activeSessions.Count; i++)
            if (activeSessions[i].seshID.Equals(mysID))
            {
                networkView.RPC("EndTheGame", activeSessions[i].p1, mysID);
                networkView.RPC("EndTheGame", activeSessions[i].p2, mysID);
